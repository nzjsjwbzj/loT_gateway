...
void HAL_UART_MspInit(UART_HandleTypeDef *huart)
{
	...
	if(huart->Instance==USART1)//???? ????1??????????1MSP?  ????
	{
    ...
    {
        ATK_MW8266D_UART_TX_GPIO_CLK_ENABLE();                              /* ???UART TX??????? */
        ATK_MW8266D_UART_RX_GPIO_CLK_ENABLE();                              /* ???UART RX??????? */
        ...
        HAL_NVIC_SetPriority(ATK_MW8266D_UART_IRQn, 6, 0);                  /* ????????6?????????0 */
        HAL_NVIC_EnableIRQ(ATK_MW8266D_UART_IRQn);                          /* ???UART??????? */
        
        __HAL_UART_ENABLE_IT(huart, UART_IT_RXNE);                          /* ???UART???????? */
        __HAL_UART_ENABLE_IT(huart, UART_IT_IDLE);                          /* ???UART??????????? */
    }
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
	 ...
	 *
	 * ??????(USART_RX_STA)???????‰Ø
	 * - bit15 (0x8000) : ???????????????????????? CR+LF ???¦Ë??
	 * - bit14 (0x4000) : ?????? 0x0D (CR) ???????????????? 0x0A (LF)
	 * - bits13..0 (0x3FFF) : ???????????????????????
	 *
	 * ?????????????
	 * 1. ??? huart->Instance == USART1 ??????????§Õ?????????? USART1 ???????
	 * 2. ???????¦Ä????USART_RX_STA & 0x8000 == 0??????????????
	 *    a) ??????????? CR??USART_RX_STA & 0x4000????
	 *       - ????????? aRxBuffer[0] ? LF (0x0A)????????? bit15(0x8000) ??¦Ë???????????????
	 *       - ????????????????§¹???????????? USART_RX_STA???????????????????????????
	 *    b) ?????¦Ä????? CR??
	 *       - ?????????? CR (0x0D)???????? CR ?????bit14????????????????? LF??
	 *       - ???????????????? USART_RX_BUF ???????? (USART_RX_STA & 0x3FFF)???????? USART_RX_STA ??????????
	 *         ????????????????????? (USART_REC_LEN - 1)???? USART_RX_STA ??¦Ë? 0???????????????????????????
	 *
	 * ????????????????
	 * - aRxBuffer[0] ??????????????????????
